    ...
    const searchPlaces = async () => {
      ...
      try {
        const response = await fetch(
          `https://google-map-places.p.rapidapi.com/maps/api/place/queryautocomplete/json?input=${encodeURIComponent(
            query
          )}&language=vi&components=country:vn&location=10.8480236,106.6895906&radius=10000`,
          {
            headers: {
    ...
    <div className="relative w-full">
      ...
      {places.length > 0 && !isSelected && (
        <div className="absolute z-10 w-full mt-1 bg-white border rounded-lg shadow-lg">
          {places.map((place, i) => {
            const key = place.place_id ?? `${place.description}-${i}`;
            const passedPlaceId = place.place_id ?? place.description ?? String(i);
            return (
              <div
                key={key}
                onClick={() => {
                  onSelectAddress(place.description, passedPlaceId);
